...
// Email delivery flag - set to false to disable email sending
const ENABLE_EMAIL = true; // Set to true to enable email delivery to sasikala.k@digitide.com
...
async function runMultipleLogins(maxRetries = 2) {
  ...
  // Generate summary
  await generateSummary(results, maxRetries);
}

async function generateSummary(results, maxRetries) {
  ...
  let emailResult = { success: false, message: 'Email delivery disabled' };
  if (ENABLE_EMAIL) {
    const emailService = new EmailService();
    emailResult = await emailService.sendTestReport({
      passed,
      failed,
      total,
      totalRetries,
  ...
  if (ENABLE_EMAIL && emailResult.success) {
    console.log(`📧 Email sent to ${emailResult.recipients} recipients`);
    console.log(`📧 Message ID: ${emailResult.messageId}`);
  } else if (ENABLE_EMAIL) {
    console.log(`❌ Email failed: ${emailResult.error}`);
  } else {
    console.log(`📧 Email delivery disabled (set ENABLE_EMAIL = true to enable)`);
  }
  console.log('='.repeat(60) + '\n');
}
